...
bool existeVueloEnc(int numVuelo)
{
    VueloParaEncomienda reg;
    int pos=0;
    while(reg.leerDeDisco(pos++))
    {
        if(numVuelo==reg.getidVuelo())
...
int setId()
{
    ...
    if(p==NULL) return -1;
    fseek(p, -sizeof(Paquete), SEEK_END);
    if(fread(&reg, sizeof(Paquete), 1, p) == 0)
    {
        id = 0;
    }
    else
    {
        id = reg.getIdpaquete()+1;
    }
    fclose(p);
    return id;
}
...
void altaPaquete()
{
    ...
    do
    {
        ...
        reg.setLargo(largo);
        reg.setPeso(peso);
        reg.setIdpaquete(id=setId());
        reg.grabarEnDisco();
        cout << " ¿DESEA AGREGAR OTRO PAQUETE? SI=1, NO=0" << endl ;
        cin >> opc;
    }
    while(opc!=0);
    cout << "EL ID DE SU PAQUETE ES : " << reg.getIdpaquete() << endl ;
}
void mostrarVuelo(char *d)
{
    cls();
    Destino reg;
    VueloParaEncomienda aux;
    int pos=0;
    char codDest[4];
    while(reg.leerDeDisco(pos++))
    {
        if(strcmp(d,reg.getDestino())==0)
        {
            strcpy(codDest,reg.getCodigoDestino());
        }
    }
    pos=0;
    while(aux.leerDeDisco(pos++))
    {
        if(strcmp(codDest,aux.getCodigoDestino())==0)
        {
            aux.mostrar();
        }
    }
...
int crearNumeroDespacho()
{
    ...
    if(p==NULL) return -1;
    fseek(p, -sizeof(Despacho), SEEK_END);
    if(fread(&reg, sizeof(Despacho), 1, p) == 0)
    {
        numDespacho = 0;
    }
    else
    {
        numDespacho = reg.getNumDespacho()+1;
    }
    fclose(p);
    return numDespacho;
}
float calcularImporte(char *dest, int idpaquete)
{
    Destino reg;
    Paquete aux;
    int pos=0,alto,largo,ancho;
    float imp=0,peso;
    while(reg.leerDeDisco(pos++))
    {
        if(strcmp(dest,reg.getDestino())==0)
        {
            imp=reg.getPrecio();
        }
    }
...
void reducirPesoDisponible(int num,int id)
{
    VueloParaEncomienda reg;
    Paquete obj;
    int pos=0;
    float peso, capacidad;
    while(obj.leerDeDisco(pos++))
    {
        if(id==obj.getIdpaquete())
        {
            peso+=obj.getPeso();
        }
    }
    ...
    while(reg.leerDeDisco(pos++))
    {
        if(reg.getidVuelo()==num)
        {
            capacidad=reg.getCapacidad();
        }
    }
    reg.setCapacidad(capacidad-peso);
    reg.grabarEnDisco(pos);
}
...
bool existeDestino(char *dest)
{
    Destino reg;
    int pos=0;
    while(reg.leerDeDisco(pos++))
    {
        if(strcmp(reg.getDestino(),dest)==0)
        {
            return true;
        }
    }
...
void confirmarDespacho()
{
    Despacho obj;
    DirEntrega aux;
    Cliente reg;
    VueloParaEncomienda obj2;
    char dni[9], dest[50];
    int id,numvuelo,num=0;
    float importe;
    bool e1,e2,e3,e4;
    cout << "INGRESE EL DNI DEL CLIENTE : " << endl ;
    cin.ignore();
    cin.getline(dni,9);
    e4=existeCliente(dni);
    if(e4==false)
    {
        return;
    }
    cout << "  A CONTINUACION LE PEDIREMOS LA DIRECCION DE ENTREGA, POR FAVOR PRESIONE INTRO " << endl ;
    aux.cargar();
    ...
    cin.ignore();
    cin.getline(dest,50);
    e2=existeDestino(dest);
    if(e2==false)
    {
        return;
    }
    cout << "SELECCIONE EL NUMERO DE VUELO PARA ENVIAR SU PAQUETE : " << endl;
    anykey();
    mostrarVuelo(dest);
    cout << endl ;
    cin >> numvuelo;
    e3=existeVuelo(numvuelo);
    if(e3==false)
    {
        return;
    }
    obj.setCliente(dni);
    obj.setPaquete(id);
    obj.setVueloparaencomienda(numvuelo);
    num=crearNumeroDespacho();
    obj.setNumDespacho(num);
    importe=calcularImporte(dest,id);
    obj.setImporte(importe);
    obj.grabarEnDisco();
    reducirPesoDisponible(numvuelo, id);
    aux.setDespacho(obj.getNumDespacho());
    aux.grabarEnDisco();
    cout << "SU NUMERO DE DESPACHO ES : " << obj.getNumDespacho() << endl ;
}
bool existeNumDespacho(int num)
{
    Despacho reg;
    int pos=0;
    while(reg.leerDeDisco(pos++))
    {
        if(num==reg.getNumDespacho())
        {
            return true;
        }
    }
...
void mostrarDespacho()
{
    Despacho obj2;
    DirEntrega reg;
    int pos=0, numDespacho;
    bool existe;
    cout << "INGRESE EL NUMERO DE DESPACHO PARA MOSTRAR EL TICKET : " << endl ;
    cin >> numDespacho;
    existe=existeNumDespacho(numDespacho);
    if(existe==false)
    {
        return;
    }
    ...
    {
        if(numDespacho==obj2.getNumDespacho())
        {
            cout << "NUMERO DE DESPACHO " << numDespacho << " : " << endl;
            obj2.mostrar();
    ...
    while(reg.leerDeDisco(pos++))
    {
        if(obj2.getNumDespacho()==reg.getDespacho())
        {
            reg.mostrar();
        }
    }
...
void promedio()
{
    ...
    Despacho aux;
    Paquete obj;
    int pos=0, con=0, c=0,numVuelo;
    float promedio,pesototal;
    cout << "INGRESE EL NUMERO DE VUELO : " << endl ;
    cin>> numVuelo;
    while(aux.leerDeDisco(pos++))
    {
        cout << "ENTRA PRIMER WHILE " << endl ;
        if(numVuelo==aux.getnumVuelo())
        {
            con=0;
            while(obj.leerDeDisco(con++))
            {
                cout << "ENTRA" << endl ;
                cout << obj.getIdpaquete() << endl ;
                cout << aux.getIdPaquete() << endl ;
                if(obj.getIdpaquete()==aux.getIdPaquete())
    ...
    cout << pesototal << endl ;
    cout << c << endl ;
    promedio=pesototal/c;
    cout << "EL PROMEDIO DE PESO DEL VUELO " << numVuelo << " ES : " << promedio;
}
